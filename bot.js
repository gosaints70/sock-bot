...
var channelsToTake = [428386306670723073, 433668422174507008, 460121919862079508, 388438789728501780, 420721621242740763];
...
client.on('ready', () => {
  ...
  /*for (var i = 0; i < charactersJson.length; i++) {
    ...
    for (var j = 0; j < creditsJson.length; j++) {
      if (creditsJson[j].wishlist.filter(f => f.id == charactersJson.findIndex(f => f.name.toLowerCase() == charactersJson[j].name.toLowerCase()) > 0))
        count++;
    }

    if (parseFloat(count) / parseFloat(creditsJson.filter(f => f.characters.length > 0).length) >= 0.5) {
      charactersJson.find(f => f.name.toLowerCase() == charactersJson[j].name.toLowerCase()).rare = true;
